...
void Database_set(Connection *conn, int id, const char *name, const char *email) {
	...
	if (addr->set) die("Id exists, unable to set");
...
int main(int argc, char *argv[]) {
	if (argc < 3) die("Usage: ex17 <db_file> <action> [params]");
	...
	if (id >= MAX_ROWS) die("Database record limit exceeded");
	...
	switch (action) {
		case 'c':
			...
			break;
		case 'g':
			if (argc != 4) die("Id required");
			Database_get(conn, id);
			break;
		case 's':
			if (argc != 6) die("Id, name, email required");
